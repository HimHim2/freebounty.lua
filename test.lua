-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL (Hop-After-Collect FIX)
-- + Fast Hop After Collect (NHỈNH NHƯNG CHỈ SAU KHI VÉT XONG DIAMONDS)
-- + Anti-DEAD + Skip Server Full + No-Rejoin-Same-Server
-- + Hop engine: Asc/Desc + cursor (gia cố) + guard lobby, grace windows
-- + Stronghold KHÔNG mở được -> Hop ngay (không đứng)
-- + Chỉ săn rương 1 & 5 gems (ưu tiên 5)
-- + Collect kim cương: Remote / Prompt / Touch + bắt spawn (emu-friendly)
...
local Config = {
    ...
    StrongholdPromptTime    = 5.0,
    StrongholdDiamondWait   = 8.0,

    FarmPlaceId             = 126509999114328, -- map farm
    LobbyCheckInterval      = 1.5,
    FarmTick                = 0.75,
    DiamondTick             = 0.25,
    ...
    -- Hop chỉ sau khi đã vét sạch
    HopAfterStronghold      = true,
    HopAfterNormalChest     = true,
    HopPostDelay            = 0.10,   -- nhịp nhanh nhưng an toàn
    ...
    ConsecutiveHopCooloff   = 5.0,
    MaxPagesPrimary         = 8,
    MaxPagesFallback        = 12,

    -- Giảm tỉ lệ hop vào server full (tự nâng slot yêu cầu tạm 60s khi fail)
    ...
    JoinGraceSeconds        = 3.2,
    FirstDiamondGrace       = 2.2,

    -- Chỉ săn rương 1 & 5 gems
    PreferChestGemCounts    = {1, 5},
    ChestScanMax            = 20,
    NoTargetHopDelay        = 0.6,
}
...
do
    ...
    task.spawn(function()
        for _, v in ipairs(g:GetDescendants()) do optimize(v); task.wait(0.0006) end
    end)
...
-- Cờ đảm bảo không hop khi còn diamonds
local HarvestInProgress = false
...
local function getItemsFolder()
    local names = {"Items","Drops","Collectibles","Loot","Pickups"}
...
local function diamondsLeft()
    local root = getItemsFolder()
    for _, v in ipairs(root:GetDescendants()) do
        if v.Name == "Diamond" and v:IsA("Model") then return true end
    end
    return false
end
local function waitAppearDiamonds(timeout)
    local t0 = os.clock()
...
-- ===== Gate: KHÔNG hop khi còn diamonds =====
local function ensureHarvestComplete(maxWaitAppear, maxWaitClear)
    -- chờ diamonds xuất hiện (nếu vừa mở rương/stronghold)
    waitAppearDiamonds(maxWaitAppear or 2.0)
    -- vét vòng cuối qua tick song song, rồi chờ sạch
    local t0 = os.clock()
    local limit = maxWaitClear or 3.5
    while (os.clock() - t0) < limit do
        if not diamondsLeft() then return true end
        task.wait(0.10)
    end
...
local function HopClassic(mode, reason)
    ...
    -- Đang thu hoạch hoặc còn diamonds → KHÔNG hop (trừ trường hợp “an toàn”)
    local safeBypass = (reason=="anti-dead" or reason=="retry" or reason=="locked")
    if (HarvestInProgress or diamondsLeft()) and not safeBypass then
        return
    end

    -- Grace
    local bypass = safeBypass or reason=="after-collect" or reason=="no-target"
    if not bypass then
        if inJoinGrace() or inFirstDiamondGrace() then return end
    end
...
-- Fast-hop: chặn hop sớm, bắt buộc sạch diamonds trước
local function HopFast(reason)
    if isTeleporting or HopRequested then return end
    if getgenv and getgenv().PauseHop then
        local t0 = os.clock()
        while getgenv().PauseHop and os.clock()-t0 < 8 do task.wait(0.2) end
    end
    HopRequested = true
    task.spawn(function()
        task.wait(Config.HopPostDelay)
        -- Nếu không phải lỗi “an toàn”, đảm bảo đã vét sạch
        if not (reason=="anti-dead" or reason=="retry" or reason=="locked") then
            ensureHarvestComplete(1.2, 3.5)
        end
        if not isTeleporting then
            pcall(function() _G._force_hop = true end)
            local why = (reason and tostring(reason):find("locked")) and "locked" or (reason or "after-collect")
...
TeleportService.TeleportInitFailed:Connect(function(_, result, msg)
    ...
    ConsecutiveHopFail = ConsecutiveHopFail + 1
    LastHopFailAt = os.clock()
    task.delay(0.2, function() HopClassic(Config.ClassicMode, "retry") end)
end)
...
-- ===== Collect diamonds (Remote/Prompt/Touch + spawn hook) =====
local DiamondSeen, LastTPBack = {}, nil
local function listDiamonds(maxCount)
    local root = getItemsFolder()
    local out, c = {}, 0
    for _, v in ipairs(root:GetDescendants()) do
        if v.Name == "Diamond" and v:IsA("Model") then
            table.insert(out, v)
            c += 1
...
local function resolveDiamondRemote()
    local RS = game:GetService("ReplicatedStorage")
    local candidates = {
        "RemoteEvents/RequestTakeDiamonds","Remotes/RequestTakeDiamonds",
        "RemoteEvents/TakeDiamonds","Remotes/TakeDiamonds",
        "RequestTakeDiamonds","TakeDiamonds",
    }
    ...
    for _, p in ipairs(candidates) do
        local obj = getPath(RS, p)
        if obj and (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) then return obj end
    end
    for _, d in ipairs({"RemoteEvents","Remotes"}) do
        local bag = RS:FindFirstChild(d)
        if bag then
            for _, obj in ipairs(bag:GetDescendants()) do
                if tostring(obj.Name):lower():find("diamond") and (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) then
                    return obj
                end
            end
        end
...
local DiamondRemote = resolveDiamondRemote()
local function tryRemoteCollect(diamond)
    if not DiamondRemote then DiamondRemote = resolveDiamondRemote() end
    local ok = false
    if DiamondRemote then
        local argsList = { {diamond}, {diamond,true}, {diamond, LocalPlayer} }
        for _, args in ipairs(argsList) do
            local succ=false
            if DiamondRemote:IsA("RemoteEvent") then
                succ = pcall(function() DiamondRemote:FireServer(unpack(args)) end)
            else
                succ = pcall(function() DiamondRemote:InvokeServer(unpack(args)) end)
            end
            if succ then ok=true break end
        end
    end
    return ok
end
local function tryPromptCollect(diamond)
    local prox = diamond:FindFirstChildWhichIsA("ProximityPrompt", true)
    if prox and prox.Enabled then
        if typeof(fireproximityprompt) == "function" then
            ...
            if hum and prox.HoldDuration == 0 then hum:MoveTo(prox.Parent and prox.Parent.Position or (HRP() and HRP().Position)); task.wait(0.05); return true end
        end
    end
    return false
end
local function tryTouchCollect(diamond)
    local hrp = HRP(); if not hrp then return false end
    local pv = diamond:GetPivot()
    local back = hrp.CFrame; LastTPBack = back
    pcall(function() hrp.CFrame = pv + Vector3.new(0, 2.2, 0) end)
    task.wait(0.05)
    pcall(function() hrp.CFrame = back end)
    return true
end
local function collectOneDiamond(diamond)
    if not (diamond and diamond.Parent) then return false end
    local id = diamond:GetDebugId()
    if DiamondSeen[id] and (os.clock() - DiamondSeen[id] < 0.4) then return false end
    DiamondSeen[id] = os.clock()
    if tryRemoteCollect(diamond) then return true end
    if tryPromptCollect(diamond) then return true end
    if tryTouchCollect(diamond) then return true end
    return false
end
local function collectAllDiamonds(maxPerTick)
    HarvestInProgress = true
    local list = listDiamonds(maxPerTick or 40)
    local got = 0
    for _, d in ipairs(list) do
        if collectOneDiamond(d) then got += 1 end
        task.wait(0.02)
    end
    if got > 0 and FirstDiamondAt == 0 then FirstDiamondAt = os.clock() end
...
task.spawn(function()
    ...
    root.DescendantAdded:Connect(function(obj)
        if obj.Name == "Diamond" and obj:IsA("Model") then
            task.wait(0.02)
            HarvestInProgress = true
            collectOneDiamond(obj)
            HarvestInProgress = false
        end
    end)
end)

-- ===== CHEST FILTER: ONLY 1 & 5 GEMS =====
local AllowedSet = {}; for _,v in ipairs(Config.PreferChestGemCounts) do AllowedSet[v]=true end
local function isPreferredGemCount(n) return n and AllowedSet[n] == true end
local function preferredChestScore(n) if n==5 then return 2 elseif n==1 then return 1 else return -1 end end
local function readChestGemCount(model)
    local candidates = {"Diamonds","Gems","GemCount","DiamondCount","Value"}
    for _, name in ipairs(candidates) do
        local val = model:FindFirstChild(name, true)
        if val and (val:IsA("IntValue") or val:IsA("NumberValue")) then
            local num = tonumber(val.Value); if num then return num end
        end
    end
    local function scanText(obj)
        local ok, text = pcall(function() return obj.Text end)
        if ok and type(text)=="string" and #text<=8 then
            local n = text:match("(%d+)"); if n then return tonumber(n) end
        end
        return nil
    end
    for _, d in ipairs(model:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            local n = scanText(d); if n then return n end
        elseif d:IsA("ProximityPrompt") then
            local ok, action = pcall(function() return d.ActionText end)
            if ok and type(action)=="string" then
                local n = action:match("(%d+)"); if n then return tonumber(n) end
            end
        end
    end
    return nil
end
local function findPreferredChest()
    ...
    for _, v in ipairs(items:GetChildren()) do
        if scanned >= Config.ChestScanMax then break end
        if v:IsA("Model") and v.Name:find("Chest") and not v.Name:find("Snow") then
            ...
            if not chestTried[id] then
                ...
                if prox and prox.Enabled then
                    scanned += 1
                    local num = readChestGemCount(v)
                    if isPreferredGemCount(num) then
                        local pv = v:GetPivot()
                        local d  = (r.Position - pv.Position).Magnitude
                        local sc = preferredChestScore(num)
                        if sc > bestScore or (sc == bestScore and d < bestDist) then
                            best, bestScore, bestDist = v, sc, d
                        end
                    end
                end
            end
...
local function bindDeathWatcher(char)
    ...
    hum.Died:Connect(function() IsDead = true; DeadSince = os.clock() end)
end
if LocalPlayer.Character then bindDeathWatcher(LocalPlayer.Character) end
LocalPlayer.CharacterAdded:Connect(function(c)
    IsDead = false; DeadSince = 0
    task.wait(0.4)
    bindDeathWatcher(c)
    if g.PlaceId == Config.FarmPlaceId then
        JoinedFarmAt = os.clock(); FirstDiamondAt = 0
    end
end)
task.spawn(function()
...
local function waitDiamonds(timeout)
    return waitAppearDiamonds(timeout or 1.6)
end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
                ...
                if not strongholdTried[sid] then
                    ...
                    if prox and prox.Enabled then
                        ...
                        if not opened and (prox and prox.Parent and prox.Enabled) then
                            ...
                            -- CHỐT: chỉ hop sau khi vét sạch
                            if waitDiamonds(Config.StrongholdDiamondWait) then
                                HarvestInProgress = true
                                collectAllDiamonds()
                                ensureHarvestComplete(0.8, 3.5)
                                HarvestInProgress = false
                                StrongholdCount = StrongholdCount + 1
                                if Config.HopAfterStronghold then HopFast("after-collect") end
                            else
                                strongholdTried[sid] = true
            ...
            -- 2) Chest 1 & 5 gems
            local chest = (function()
                -- tìm rương theo filter
                local r = HRP(); if not r then return nil end
                local items = workspace:FindFirstChild("Items"); if not items then return nil end
                local AllowedSet = {}; for _,v in ipairs(Config.PreferChestGemCounts) do AllowedSet[v]=true end
                local function isPreferred(n) return n and AllowedSet[n]==true end
                local function score(n) if n==5 then return 2 elseif n==1 then return 1 else return -1 end end
                local function readCount(m)
                    local names = {"Diamonds","Gems","GemCount","DiamondCount","Value"}
                    for _, nm in ipairs(names) do
                        local val = m:FindFirstChild(nm, true)
                        if val and (val:IsA("IntValue") or val:IsA("NumberValue")) then
                            local num = tonumber(val.Value); if num then return num end
                        end
                    end
                ...
                for _, v in ipairs(items:GetChildren()) do
                    ...
                    if v:IsA("Model") and v.Name:find("Chest") and not v.Name:find("Snow") then
                        ...
                        if not chestTried[id] then
                            ...
                            if prox and prox.Enabled then
                                scanned += 1
                                local num = readCount(v)
                                if isPreferred(num) then
                                    local pv = v:GetPivot()
                                    local d  = (r.Position - pv.Position).Magnitude
                                    local sc = score(num)
                                    if sc > bestScore or (sc == bestScore and d < bestDist) then
            ...
            if not chest then
                -- KHÔNG rương mục tiêu: chỉ hop khi KHÔNG còn diamonds
                if not diamondsLeft() then
                    task.wait(Config.NoTargetHopDelay)
                    if not diamondsLeft() then HopFast("no-target") end
                end
            else
                ...
                local t0   = os.clock()
                local okOpen = false
                while prox and prox.Parent and prox.Enabled and (os.clock() - t0) < 8.5 do
                ...
                if okOpen then
                    ...
                    -- chờ kim cương xuất hiện rồi vét sạch
                    waitAppearDiamonds(1.6)
                    collectAllDiamonds()
                    ensureHarvestComplete(0.8, 3.5)
                    HarvestInProgress = false
                    if Config.HopAfterNormalChest then HopFast("after-collect") end
                else
                    chestTried[id] = true
...
-- Diamonds loop song song (nhẹ)
task.spawn(function()
    while task.wait(Config.DiamondTick) do
        if g.PlaceId == Config.FarmPlaceId then
            HarvestInProgress = true
            collectAllDiamonds(20)
            HarvestInProgress = false
        end
    end
end)
...
task.spawn(function()
    ...
    local function rainbowStroke(stroke)
        task.spawn(function()
            while task.wait() do
                for i = 0, 1, 0.02 do stroke.Color = Color3.fromHSV(i, 1, 1); task.wait(0.03) end
            end
        end)
    end
    rainbowStroke(d)
    ...
    e.Text = "0"; e.TextColor3 = Color3.fromRGB(255,255,255); e.Font = Enum.Font.GothamBold
    e.TextScaled = true; e.TextStrokeTransparency = 0.6
