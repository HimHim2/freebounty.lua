-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL (1/5 Gems Priority)
...
-- + ƯU TIÊN & CHỈ LẤY: RƯƠNG 1 GEM & 5 GEMS (bỏ qua rương khác)  <-- CHANGED
-- + Emu-friendly: nhịp wait an toàn, không spam nặng event loop    <-- NEW
...
local Config = {
    ...
    StrongholdPromptTime    = 5.0,     -- CHANGED: ép nhanh hơn, giảm 6 -> 5
    StrongholdDiamondWait   = 8.0,     -- CHANGED: giảm chờ xuất kim cương để tăng nhịp hop

    FarmPlaceId             = 126509999114328, -- map farm
    LobbyCheckInterval      = 1.5,     -- CHANGED: nhịp lobby nhanh hơn 2.0 -> 1.5
    FarmTick                = 0.75,    -- CHANGED: vòng farm nhanh hơn (nhưng an toàn cho emu)
    DiamondTick             = 0.25,    -- CHANGED: vét kim cương mượt hơn

    HopBackoffMin           = 0.9,     -- CHANGED: giảm backoff để hop nhanh hơn
    HopBackoffMax           = 1.7,     -- CHANGED

    -- Fast hop sau khi NHẶT xong
    HopAfterStronghold      = true,
    HopAfterNormalChest     = true,
    HopPostDelay            = 0.12,    -- CHANGED: đẩy nhanh post-delay

    -- Anti-DEAD
    DeadHopTimeout          = 5.0,     -- CHANGED: phản ứng nhanh hơn giả lập/PC
    DeadUiKeywords          = { "dead", "you died", "respawn", "revive" },

    -- Harden
    MaxConsecutiveHopFail   = 5,
    ConsecutiveHopCooloff   = 5.0,     -- CHANGED: cooloff ngắn hơn
    MaxPagesPrimary         = 8,
    MaxPagesFallback        = 12,
    ...
    JoinGraceSeconds        = 3.2,     -- CHANGED: rút ngắn thời gian “ở lại”
    FirstDiamondGrace       = 2.2,     -- CHANGED

    -- ====== CHỈ SĂN RƯƠNG 1 GEM & 5 GEMS ======
    -- Cách nhận diện: ưu tiên Value objects/thuộc tính hiển thị số diamonds,
    -- fallback dò text trong UI/Prompt, nếu không xác định được thì bỏ qua.
    PreferChestGemCounts    = {1, 5},  -- NEW
    ChestScanMax            = 20,      -- NEW: tối đa số rương xét trong 1 vòng để nhẹ emu
    NoTargetHopDelay        = 0.6,     -- NEW: không có rương mục tiêu -> hop nhanh
}
...
do
    ...
    task.spawn(function()
        for _, v in ipairs(g:GetDescendants()) do optimize(v); task.wait(0.0006) end -- CHANGED: nhịp nhẹ
    end)
...
local function HopFast(reason)
    ...
    if getgenv and getgenv().PauseHop then
        local t0 = os.clock()
        while getgenv().PauseHop and os.clock()-t0 < 8 do task.wait(0.2) end -- CHANGED: nhẹ hơn
    end
    HopRequested = true
    task.spawn(function()
...
-- ====== ONLY 1 & 5 GEMS CHESTS (PREFERRED) ======================
-- Nhiều map/model dùng Value/Object để chứa số gem hoặc hiển thị trong GUI.
-- Ta cố gắng đọc các khả năng phổ biến; nếu không xác định được -> bỏ qua.
local function readChestGemCount(model)
    -- 1) Tìm IntValue/NumberValue tên tương đối
    ...
    -- 2) Dò text trong descendants (Billboard/TextLabel/Prompt)
...
local function isPreferredGemCount(n)
    return n and AllowedSet[n] == true
end

local function preferredChestScore(n)
    -- ƯU TIÊN 5 trước rồi tới 1 (điểm cao hơn = ưu tiên hơn)
            ...
            if not chestTried[id] then
                ...
                if prox and prox.Enabled then
                    ...
                    if isPreferredGemCount(num) then
                        ...
                        -- chọn rương điểm cao nhất, nếu bằng điểm -> chọn gần hơn
...
local function pressPromptWithTimeout(prompt, timeout)
    ...
    while prompt and prompt.Parent and prompt.Enabled and (os.clock()-t0) < limit do
        ...
        task.wait(0.25) -- CHANGED: nhanh hơn nhưng an toàn trên emu
    end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- 2) Chest thường: CHỈ rương 1 & 5 gems  ----------------- CHANGED
            local chest = findPreferredChest()
            if not chest then
                -- Không có rương mục tiêu ⇒ hop sớm để tiết kiệm thời gian
                ...
                while prox and prox.Parent and prox.Enabled and (os.clock() - t0) < 8.5 do -- CHANGED: timeout nhanh
                    tpCFrame(CFrame.new(chest:GetPivot().Position))
                    firePromptSafe(prox)
                    task.wait(0.35) -- CHANGED: nhịp bấm gọn hơn
