-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL v2
...
-- + No-Progress Watchdog: không nhặt/không spawn kim cương -> hop
-- + Stabilized cho emulator + queue_on_teleport + error prompt hook
...
local Config = {
    ...
    StrongholdPromptTime    = 6,    -- NHẤN mở stronghold tối đa 6s, không mở được -> hop
    StrongholdDiamondWait   = 10,   -- Đợi diamonds spawn sau khi mở stronghold

    FarmPlaceId             = 126509999114328, -- map farm
    LobbyCheckInterval      = 2.0,
    FarmTick                = 0.75, -- nhanh hơn 1 chút nhưng không spam
    DiamondTick             = 0.35,
    ...
    -- Giảm join server full (tự tăng yêu cầu slot tạm thời khi fail)
    ...
    -- Watchdog: không tiến triển thì hop (trị đứng yên / map toàn rương khoá / không spawn)
    NoProgressTimeout       = 25.0,  -- không nhặt được viên nào trong ~25s => hop
    NoChestTimeout          = 14.0,  -- không tìm thấy chest usable trong ~14s => hop
    NoDiamondAppearTimeout  = 8.0,   -- mở chest mà không thấy diamond spawn => hop

    -- Safety: tránh spam RequestTakeDiamonds
    CollectBurstMax         = 75,    -- tối đa mỗi burst
    CollectBurstDelay       = 0.15,  -- delay giữa các burst
}
...
local function getEffectiveMinFreeSlots()
    ...
    return clamp(DynamicMinFreeSlots, Config.MinFreeSlotsDefault, Config.MinFreeSlotsCeil)
end
...
local function HopClassic(mode, reason)
    ...
    local bypass = false
    if reason == "after-collect" or reason == "anti-dead" or reason == "retry" or reason == "locked" or reason=="watchdog" then
        bypass = true
    end
    if not bypass then
...
local function HopFast(reason)
    ...
    task.spawn(function()
        ...
        if not isTeleporting then
            ...
            local why = (reason and tostring(reason):find("locked")) and "locked" or ((reason=="watchdog") and "watchdog" or "after-collect")
...
local function collectAllDiamonds()
    ...
    if not (re and re.FireServer) then return 0 end
    ...
    for _, v in ipairs(items:GetChildren()) do
        if v.Name == "Diamond" then
            ...
            n += 1
            burst += 1
            if burst >= Config.CollectBurstMax then
                task.wait(Config.CollectBurstDelay)
                burst = 0
            end
        end
    end
    if n > 0 then
        LastCollectAt = os.clock()
        if FirstDiamondAt == 0 then FirstDiamondAt = LastCollectAt end
    end
    return n
end
...
local LastChestSeenAt = 0
local function markProgress()
    LastProgressAt = os.clock()
end
local function markChestSeen()
    LastChestSeenAt = os.clock()
end

task.spawn(function()
    while task.wait(1.0) do
        if g.PlaceId == Config.FarmPlaceId then
            -- Nếu lâu không nhặt được viên nào
            if LastCollectAt > 0 then
                markProgress()
            end
            if (LastProgressAt > 0) and ((os.clock() - LastProgressAt) > Config.NoProgressTimeout) then
                HopFast("watchdog") -- hop vì không có tiến triển
            end
            -- Nếu lâu không thấy chest usable
            if (LastChestSeenAt > 0) and ((os.clock() - LastChestSeenAt) > Config.NoChestTimeout) then
                HopFast("watchdog")
            end
        end
    end
    ...
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
                    ...
                    if prox and prox.Enabled then
                        ...
                        if not opened and (prox and prox.Parent and prox.Enabled) then
                            ...
                            if waitDiamonds(Config.StrongholdDiamondWait) then
                                local n = collectAllDiamonds()
                                if n > 0 then markProgress() end
                                waitNoDiamonds(1.1)
                                StrongholdCount += 1
            ...
            if chest then
                ...
                -- giữ vị trí ổn định trước khi nhấn
                local pivot = chest:GetPivot()
                tpCFrame(CFrame.new(pivot.Position + Vector3.new(0, 2.8, 0)))

                while prox and prox.Parent and prox.Enabled and (os.clock() - t0) < 8.0 do
                    firePromptSafe(prox)
                    task.wait(0.35)
                    prox = chest:FindFirstChildWhichIsA("ProximityPrompt", true)
                end

                local opened = not (prox and prox.Parent and prox.Enabled)
                if opened then
                    -- đợi diamond xuất hiện (nếu map delay)
                    if not waitDiamonds(Config.NoDiamondAppearTimeout) then
                        chestTried[id] = true
                        HopFast("chest-no-diamonds")
                    else
                        local n = collectAllDiamonds()
                        if n > 0 then markProgress() end
                        waitNoDiamonds(1.0)
                ...
                -- KHÔNG chest usable ngay lúc này: KHÔNG hop liền để tránh hop sớm.
                -- Watchdog sẽ lo nếu lâu quá không có chest hoặc không có progress.
            end
        end
    end
end)
...
-- ===== GemsCap writer (giữ nguyên logic bạn đưa) =====
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local function _gems()
    local pg = LocalPlayer:FindFirstChild("PlayerGui"); if not pg then return 0 end
    local ui = pg:FindFirstChild("Interface"); if not ui then return 0 end
    local dc = ui:FindFirstChild("DiamondCount"); if not dc then return 0 end
    local lab = dc:FindFirstChild("Count"); if not (lab and lab:IsA("TextLabel")) then return 0 end
    local s = tostring(lab.Text):gsub("[,%s]", "")
...
task.spawn(function()
    while task.wait(1.5) do
        local cap = (getgenv() and getgenv().CCHUB and tonumber(getgenv().CCHUB.GemsCap)) or math.huge
        local gms = _gems()
        if gms >= cap and not _wrote then
            _wrote = true
            local filename = (LocalPlayer.Name or "player") .. ".txt"
