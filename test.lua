-- ================================================================
-- Mochi Farm (Luarmor-ready & Emulator-stable, no UI) - FULL MERGED
...
-- + ƯU TIÊN & CHỈ LẤY: RƯƠNG 1 GEM & 5 GEMS
-- + Gom kim cương siêu bền: Remote / Prompt / Touch + bắt spawn
-- + Emu-friendly: nhịp nhẹ, không spam event loop
...
local function HopClassic(mode, reason)
    ...
    -- Grace windows
    local bypass = false
    if reason == "after-collect" or reason == "anti-dead" or reason == "retry" or reason == "locked" or reason=="no-target" then
        bypass = true
    end
    if not bypass then
...
-- ================================================================
-- ===== DIAMOND HELPERS (RELIABLE COLLECT) - MERGED
-- ================================================================
local DiamondSeen = {}   -- debounce per-instance
local LastTPBack  = nil  -- lưu vị trí về sau khi chạm diamond

-- Tìm folder chứa diamonds (Items/Drops/Collectibles/…)
...
-- Chuẩn hoá danh sách viên kim cương
...
-- Tìm Remote (Event/Function) dùng để nhặt
local function resolveDiamondRemote()
    local RS = game:GetService("ReplicatedStorage")
    local candidates = {
        "RemoteEvents/RequestTakeDiamonds",
        "Remotes/RequestTakeDiamonds",
        "RemoteEvents/TakeDiamonds",
        "Remotes/TakeDiamonds",
        "RequestTakeDiamonds",
        "TakeDiamonds",
    }
...
-- Gửi yêu cầu nhặt (Event/Function) với nhiều kiểu tham số thường gặp
...
-- Fallback 1: nếu viên có ProximityPrompt
    ...
    if prox and prox.Enabled then
        if typeof(fireproximityprompt) == "function" then
            ...
            if hum and prox.HoldDuration == 0 then
                hum:MoveTo(prox.Parent and prox.Parent.Position or (HRP() and HRP().Position))
                task.wait(0.05)
                return true
            end
        end
...
-- Fallback 2: Touch-collect (dịch HRP chấm nhẹ vào hitbox viên kim cương)
...
-- TRUE nếu còn diamonds trong map
...
-- Đợi cho tới khi hết diamonds hoặc hết thời gian
...
local function collectOneDiamond(diamond)
    ...
    if DiamondSeen[id] and (os.clock() - DiamondSeen[id] < 0.4) then
        return false
    end
...
-- Thu gom tất cả diamonds hiện có
local function collectAllDiamonds(maxPerTick)
    local list = listDiamonds(maxPerTick or 40)
    local got = 0
    for _, d in ipairs(list) do
        if collectOneDiamond(d) then
            got += 1
        end
        task.wait(0.02)
    end
    if got > 0 and FirstDiamondAt == 0 then
        FirstDiamondAt = os.clock()
    end
    return got
end

-- Khi Diamond spawn → gom ngay (giảm độ trễ)
task.spawn(function()
    local root = getItemsFolder()
...
-- ================================================================
local AllowedSet = {}
for _,v in ipairs(Config.PreferChestGemCounts) do AllowedSet[v]=true end
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            -- 2) Chest thường: CHỈ rương 1 & 5 gems
            local chest = findPreferredChest()
            if not chest then
