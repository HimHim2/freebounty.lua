...
-- + Bản vá tương thích map đổi cấu trúc (Items/Remote/Teleporter)
-- ================================================================
...
local Config = {
    ...
    StrongholdChestName     = "Stronghold Diamond Chest", -- tên gốc; có fuzzy match ở dưới
    ...
    -- Giảm tỉ lệ hop vào server full (tăng yêu cầu slot tạm khi fail)
...
-- ===== GENERIC HELPERS (Fuzzy path) =====
local function findItemsFolder()
    local cand = {
        workspace:FindFirstChild("Items"),
        workspace:FindFirstChild("Drops"),
        workspace:FindFirstChild("ItemDrops"),
        workspace:FindFirstChild("WorldItems"),
    }
    for _, f in ipairs(cand) do if f and f:IsA("Folder") then return f end end
    -- last resort: scan nông
    for _, v in ipairs(workspace:GetChildren()) do
        if v:IsA("Folder") and v.Name:lower():find("item") then return v end
    end
    return nil
end

local function safeIdOf(inst)
    if not inst then return nil end
    local id
    pcall(function() id = inst:GetDebugId() end)
    if id and type(id)=="string" and #id>0 then return id end
    pcall(function() id = inst:GetFullName() end)
    if id and type(id)=="string" and #id>0 then return id end
...
local function getDiamondRemote()
    local function findIn(container)
        ...
        for _, v in ipairs(container:GetDescendants()) do
            if v:IsA("RemoteEvent") then
                ...
                if n:find("diamond") or n:find("diam") then
                    return v
                end
            end
        end
    ...
    -- các đường dẫn khả dĩ
    local candidates = {
        RS:FindFirstChild("RemoteEvents"),
        RS:FindFirstChild("RemoteEvent"),
...
local _DiamondRemote = getDiamondRemote()

local function diamondsLeft()
    local items = findItemsFolder()
...
local function collectAllDiamonds()
    ...
    for _, v in ipairs(items:GetChildren()) do
        if v.Name == "Diamond" then
            pcall(function()
                if not _DiamondRemote or not _DiamondRemote.Parent then
                    _DiamondRemote = getDiamondRemote()
                end
                if _DiamondRemote and _DiamondRemote.FireServer then
                    _DiamondRemote:FireServer(v)
                    n = n + 1
                end
            end)
        end
...
local function findStrongholdChest()
    ...
    local exact = items:FindFirstChild(Config.StrongholdChestName)
    if exact then return exact end

    -- Fuzzy: chứa “stronghold” và “chest”
    local function isStrongholdName(n)
        n = tostring(n):lower()
        return (n:find("stronghold") ~= nil) and (n:find("chest") ~= nil)
    end
    for _, v in ipairs(items:GetChildren()) do
        if v:IsA("Model") and isStrongholdName(v.Name) then
            return v
        end
    end
    return nil
...
local function findPromptAnywhere(root)
    if not root then return nil end
    local prox = root:FindFirstChildWhichIsA("ProximityPrompt", true)
    if prox then return prox end
    -- một số map để trong Attachment / ProximityInteraction
...
local function firePromptSafe(prompt)
    if typeof(fireproximityprompt) == "function" then
        ...
        -- fallback rất nhẹ nhàng
        pcall(function()
            local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
            if hum and prompt and prompt.HoldDuration == 0 then
                hum:MoveTo(prompt.Parent and (prompt.Parent.Position or prompt.Parent.CFrame.Position) or (HRP() and HRP().Position))
            end
        end)
    end
end
...
local function parsePlayersText(raw)
    if type(raw) ~= "string" then return nil, nil end
    local a,b = raw:match("(%d+)%s*/%s*(%d+)")
    if not a then a,b = raw:match("(%d+)%s*%-?%s*(%d+)") end
...
local function isTeleporterModel(m)
    ...
    if name:find("teleporter") or name:find("portal") or name:find("gate") then
        return true
    end
    return (m:FindFirstChild("BillboardGui") ~= nil) or (m:FindFirstChildWhichIsA("BillboardGui", true) ~= nil)
end

local function teleporterEnterPart(m)
    local p = m and (m:FindFirstChildWhichIsA("BasePart") or m:FindFirstChild("Part"))
    return p
end
...
local function autoJoinOrCreate()
    ...
    for _, obj in ipairs(workspace:GetChildren()) do
        if isTeleporterModel(obj) then
            local board = obj:FindFirstChildWhichIsA("BillboardGui", true)
            local lab = board and (board:FindFirstChild("Players") or board:FindFirstChildWhichIsA("TextLabel", true))
            local txt = lab and lab.Text
            local x, y = parsePlayersText(txt or "")
            if x and y and x >= 2 and x < y then
                local enter = teleporterEnterPart(obj)
    ...
    if not joined then
        -- không có phòng đang lập -> tự bước vào cổng bất kỳ để create
        for _, obj in ipairs(workspace:GetChildren()) do
            if isTeleporterModel(obj) then
                local enter = teleporterEnterPart(obj)
                if enter and HRP() then
    ...
    for _, gui in ipairs(pg:GetDescendants()) do
        if gui:IsA("TextLabel") or gui:IsA("TextButton") then
            ...
            for _, k in ipairs(Config.DeadUiKeywords) do
                if t:find(lower(k), 1, true) then return true end
            end
        end
    end
    return false
...
task.spawn(function()
    while task.wait(Config.FarmTick) do
        if g.PlaceId == Config.FarmPlaceId then
            ...
            if sh then
                local sid = safeIdOf(sh)
                if not strongholdTried[sid] then
                    local prox = findProximityPromptInChest(sh)
                    if prox and prox.Enabled then
                        local pv = pcall(function() return sh:GetPivot().Position end) and sh:GetPivot().Position or sh.PrimaryPart and sh.PrimaryPart.Position or HRP() and HRP().Position or Vector3.new()
                        tpCFrame(CFrame.new(pv + Vector3.new(0,3,0)))
            ...
            if not chest then
                ...
                local id   = safeIdOf(chest)
                local prox = findPromptAnywhere(chest)
                local t0   = os.clock()
                local okOpen = false

                while prox and prox.Parent and prox.Enabled and (os.clock() - t0) < 10 do
                    local pv = pcall(function() return chest:GetPivot().Position end) and chest:GetPivot().Position or chest.PrimaryPart and chest.PrimaryPart.Position or HRP() and HRP().Position or Vector3.new()
                    tpCFrame(CFrame.new(pv))
                    firePromptSafe(prox)
                    task.wait(0.45)
                    prox = findPromptAnywhere(chest)
                end
